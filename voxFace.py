...
class Main:
    ...
    def importSprites(self):
        ...
        self.importSmirkSprites()
        self.setLoadingScreenAt(0.35)
        ...
        self.importFlabbergastedSprites()
        self.setLoadingScreenAt(1)
    ...
    def importSmirkSprites(self):
        # EYES
        ## Left eye
        self.smirkLeftEyeMask = pygame.image.load("source/vox_smirk_lefteye_mask.svg").convert_alpha()
        self.smirkLeftEyeMask = pygame.transform.scale(self.smirkLeftEyeMask, self.ur([744, 389], x=True, y=True))
        self.smirkLeftEyeImage = pygame.Surface(self.smirkLeftEyeMask.get_size())

        ## Left eye border
        self.smirkLeftEyeBorder = pygame.image.load("source/vox_smirk_lefteye_border.svg").convert_alpha()
        self.smirkLeftEyeBorder = pygame.transform.scale(self.smirkLeftEyeBorder, self.ur([744, 389], x=True, y=True))

        ## Left eyebrow
        self.smirkLeftEyebrow = pygame.image.load("source/vox_smirk_lefteyebrow.svg").convert_alpha()
        self.smirkLeftEyebrow = pygame.transform.scale(self.smirkLeftEyebrow, self.ur([663, 398], x=True, y=True))

        ## Right eye
        self.smirkRightEyeMask = pygame.image.load("source/vox_smirk_righteye_mask.svg").convert_alpha()
        self.smirkRightEyeMask = pygame.transform.scale(self.smirkRightEyeMask, self.ur([779, 330], x=True, y=True))
        self.smirkRightEyeImage = pygame.Surface(self.smirkRightEyeMask.get_size())

        ## Right eye border
        self.smirkRightEyeBorder = pygame.image.load("source/vox_smirk_righteye_border.svg").convert_alpha()
        self.smirkRightEyeBorder = pygame.transform.scale(self.smirkRightEyeBorder, self.ur([779, 330], x=True, y=True))

        ## Right eyebrow
        self.smirkRightEyebrow = pygame.image.load("source/vox_smirk_righteyebrow.svg").convert_alpha()
        self.smirkRightEyebrow = pygame.transform.scale(self.smirkRightEyebrow, self.ur([743, 386], x=True, y=True))

        # MOUTH
        self.smirkMouthMask = pygame.image.load("source/vox_smirk_mouth_mask.svg").convert_alpha()
        self.smirkMouthMask = pygame.transform.scale(self.smirkMouthMask, self.ur([1328, 876], x=True, y=True))
        self.smirkMouthImage = pygame.image.load("source/vox_smirk_mouth_bg.svg").convert_alpha()
        self.smirkMouthImage = pygame.transform.scale(self.smirkMouthImage, self.ur([1328, 876], x=True, y=True))

        # Mouth borders
        self.smirkMouthTop = pygame.image.load("source/vox_smirk_mouth_top.svg").convert_alpha()
        self.smirkMouthTop = pygame.transform.scale(self.smirkMouthTop, self.ur([1706, 513], x=True, y=True))
        self.smirkMouthBorder = pygame.image.load("source/vox_smirk_mouth_border.svg").convert_alpha()
        self.smirkMouthBorder = pygame.transform.scale(self.smirkMouthBorder, self.ur([1328, 876], x=True, y=True))
    ...
    def importFlabbergastedSprites(self):
        # EYES
        ## Left eye
        self.fgLeftEyeMask = pygame.image.load("source/vox_fg_lefteye_mask.svg").convert_alpha()
        self.fgLeftEyeMask = pygame.transform.scale(self.fgLeftEyeMask, self.ur([827, 566], x=True, y=True))
        self.fgLeftEyeImage = pygame.Surface(self.fgLeftEyeMask.get_size())

        ## Left eye border
        self.fgLeftEyeBorder = pygame.image.load("source/vox_fg_lefteye_border.svg").convert_alpha()
        self.fgLeftEyeBorder = pygame.transform.scale(self.fgLeftEyeBorder, self.ur([827, 566], x=True, y=True))

        ## Left eyebrow
        self.fgLeftEyebrow = pygame.image.load("source/vox_fg_lefteyebrow.svg").convert_alpha()
        self.fgLeftEyebrow = pygame.transform.scale(self.fgLeftEyebrow, self.ur([768, 222], x=True, y=True))

        ## Left eye dark circle
        self.fgLeftDarkCircle = pygame.image.load("source/vox_fg_leftdarkcircle.svg").convert_alpha()
        self.fgLeftDarkCircle = pygame.transform.scale(self.fgLeftDarkCircle, self.ur([163, 150], x=True, y=True))

        ## Right eye
        self.fgRightEyeMask = pygame.image.load("source/vox_fg_righteye_mask.svg").convert_alpha()
        self.fgRightEyeMask = pygame.transform.scale(self.fgRightEyeMask, self.ur([822, 566], x=True, y=True))
        self.fgRightEyeImage = pygame.Surface(self.fgRightEyeMask.get_size())

        ## Right eye border
        self.fgRightEyeBorder = pygame.image.load("source/vox_fg_righteye_border.svg").convert_alpha()
        self.fgRightEyeBorder = pygame.transform.scale(self.fgRightEyeBorder, self.ur([822, 566], x=True, y=True))

        ## Right eyebrow
        self.fgRightEyebrow = pygame.image.load("source/vox_fg_righteyebrow.svg").convert_alpha()
        self.fgRightEyebrow = pygame.transform.scale(self.fgRightEyebrow, self.ur([760, 222], x=True, y=True))

        ## Right eye dark circle
        self.fgRightDarkCircle = pygame.image.load("source/vox_fg_rightdarkcircle.svg").convert_alpha()
        self.fgRightDarkCircle = pygame.transform.scale(self.fgRightDarkCircle, self.ur([159, 150], x=True, y=True))

        ## Pupil
        self.fgPupil = pygame.image.load("source/vox_fg_pupil.svg").convert_alpha()
        self.fgPupil = pygame.transform.scale(self.fgPupil, self.ur([59, 57], x=True, y=True))

        # MOUTH
        self.fgMouthMask = pygame.image.load("source/vox_fg_mouth_mask.svg").convert_alpha()
        self.fgMouthMask = pygame.transform.scale(self.fgMouthMask, self.ur([1405, 344], x=True, y=True))
        self.fgMouthImage = pygame.image.load("source/vox_fg_mouth_bg.svg").convert_alpha()
        self.fgMouthImage = pygame.transform.scale(self.fgMouthImage, self.ur([1405, 344], x=True, y=True))

        ## Mouth border
        self.fgMouthBorder = pygame.image.load("source/vox_fg_mouth_border.svg").convert_alpha()
        self.fgMouthBorder = pygame.transform.scale(self.fgMouthBorder, self.ur([1405, 344], x=True, y=True))

        ## Teeth
        self.fgTeethTop = pygame.image.load("source/vox_fg_upteeth.svg").convert_alpha()
        self.fgTeethTop = pygame.transform.scale(self.fgTeethTop, self.ur([1287, 306], x=True, y=True))
        self.fgTeethBottom = pygame.image.load("source/vox_fg_bottomteeth.svg").convert_alpha()
        self.fgTeethBottom = pygame.transform.scale(self.fgTeethBottom, self.ur([1405, 344], x=True, y=True))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                match event.key:
                    ...
                    case pygame.K_4 :
                        self.currentState = voxState.SMIRK
                    case pygame.K_5 :
                    ...
                    case pygame.K_RIGHTPAREN :
                        self.currentState = voxState.FLABBERGASTED
                    case pygame.K_z :
    ...
    def display(self):
        ...
        match self.currentState:
            ...
            case voxState.SMOL:
                self.showSmolFace()
            case voxState.FLABBERGASTED:
                self.showFlabbergasterFace()
            case voxState.MUTED:
                self.showMutedFace()
    ...
    def showDefaultFace(self):
        # LEFT EYE
        pupilPosition = [self.defaultLeftEyeImage.get_width() / 2, self.defaultLeftEyeImage.get_height() / 2]
        pupilPosition[0] += self.eyeShiftRatioX * (self.defaultLeftEyeImage.get_width() / 3) + self.defaultPupilImage.get_width()
        pupilPosition[1] += self.eyeShiftRatioY * (self.defaultLeftEyeImage.get_height() / 3)
        ...
        # RIGHT EYE
        pupilPosition = [self.defaultRightEyeImage.get_width() / 2, self.defaultRightEyeImage.get_height() / 2]
        pupilPosition[0] += self.eyeShiftRatioX * (self.defaultRightEyeImage.get_width() / 3) - self.defaultPupilImage.get_width()
        pupilPosition[1] += self.eyeShiftRatioY * (self.defaultRightEyeImage.get_height() / 3)
    ...
    def showBlazedFace(self):
        ...
        position = self.ur([736, 676], x=True, y=True)
    ...
    def showSmirkFace(self):
        # LEFT EYE
        pupilPosition = [self.smirkLeftEyeImage.get_width() / 2, self.smirkLeftEyeImage.get_height() / 2]
        pupilPosition[0] += self.eyeShiftRatioX * (self.smirkLeftEyeImage.get_width() / 4) + self.defaultPupilImage.get_width()
        pupilPosition[1] += self.eyeShiftRatioY * (self.smirkLeftEyeImage.get_height() / 4)
        ...
        self.smirkLeftEyeImage.fill((255, 0, 66))
        self.smirkLeftEyeImage.blit(finalPupilImage, pupilPosition)
        self.smirkLeftEyeImage.blit(self.smirkLeftEyeBorder, (0,0))

        result = self.smirkLeftEyeMask.copy()
        result.blit(self.smirkLeftEyeImage, (0, 0), None, pygame.BLEND_MULT)

        position = self.ur([234, 160], x=True, y=True)
        position[1] += faceDisplacement
        self.screen.blit(result, position)

        position = self.ur([356, 54], x=True, y=True)
        position[1] += faceDisplacement
        self.screen.blit(self.smirkLeftEyebrow, position)


        # RIGHT EYE
        pupilPosition = [self.smirkRightEyeImage.get_width() / 2, self.smirkRightEyeImage.get_height() / 2]
        pupilPosition[0] += self.eyeShiftRatioX * (self.smirkLeftEyeImage.get_width() / 4) - self.defaultPupilImage.get_width()
        pupilPosition[1] += self.eyeShiftRatioY * (self.smirkLeftEyeImage.get_height() / 4) - self.defaultPupilImage.get_height()/2

        self.smirkRightEyeImage.fill((255, 0, 66))
        self.smirkRightEyeImage.blit(finalPupilImage, pupilPosition)
        self.smirkRightEyeImage.blit(self.smirkRightEyeBorder, (0,0))

        result = self.smirkRightEyeMask.copy()
        result.blit(self.smirkRightEyeImage, (0, 0), None, pygame.BLEND_MULT)

        position = self.ur([1089, 253], x=True, y=True)
        position[1] += faceDisplacement
        self.screen.blit(result, position)

        position = self.ur([1089, 66], x=True, y=True)
        position[1] += faceDisplacement
        self.screen.blit(self.smirkRightEyebrow, position)


        # MOUTH
        mouthImage = self.smirkMouthImage.copy()
        mouthImage.blit(self.smirkMouthBorder, (0,0))

        newMouthImage = self.smirkMouthMask.copy()
        newMouthImage.blit(mouthImage, (0, 0), None, pygame.BLEND_RGBA_MULT)

        scale = [ newMouthImage.get_width(), newMouthImage.get_height() ]
        scale[1] *= self.mouthShiftRatio
        newMouthImage = pygame.transform.scale(newMouthImage, scale)

        finalMouthImage = self.smirkMouthMask.copy()
        finalMouthImage.blit(newMouthImage.copy(), (0, 0), None, pygame.BLEND_RGBA_MULT)
        finalMouthImage.set_colorkey((255,255,255))

        result = pygame.Surface(self.ur([1706, 1088], x=True, y=True), pygame.SRCALPHA)
        result.blit(finalMouthImage, self.ur([143, 215], x=True, y=True))
        result.blit(self.smirkMouthTop, (0,0))

        scale = [ result.get_width(), result.get_height() ]
        scale[0] *= 1 - ( self.mouthShiftRatio * .5 )
        scale[1] *= 1 - ( self.mouthShiftRatio * .7 )
        result = pygame.transform.scale(result, scale)

        position = self.ur([167, 418], x=True, y=True)
        position[0] += self.smirkMouthTop.get_width()/2
        position[0] -= result.get_width()/2
        position[1] += faceDisplacement
    ...
    def showFlabbergasterFace(self):
        # LEFT EYE
        pupilPosition = [self.fgLeftEyeMask.get_width()/2, self.fgLeftEyeMask.get_height()/2]
        pupilPosition[0] += self.eyeShiftRatioX * (self.fgLeftEyeMask.get_width() / 4) + self.fgPupil.get_width()
        pupilPosition[1] += self.eyeShiftRatioY * (self.fgLeftEyeMask.get_height() / 5) - self.fgPupil.get_height()

        faceDisplacement = self.eyeShiftRatioY * self.ur(80, y=True)

        self.fgLeftEyeImage.fill((255, 0, 66))
        self.fgLeftEyeImage.blit(self.fgPupil, pupilPosition)
        self.fgLeftEyeImage.blit(self.fgLeftEyeBorder, (0,0))

        result = self.fgLeftEyeMask.copy()
        result.blit(self.fgLeftEyeImage, (0, 0), None, pygame.BLEND_RGBA_MULT)

        position = self.ur([133,175], x=True, y=True)
        position[1] += faceDisplacement
        self.screen.blit(result, position)

        position = self.ur([133, -45], x=True, y=True)
        position[1] += faceDisplacement
        self.screen.blit(self.fgLeftEyebrow, position)

        position = self.ur([786, 553], x=True, y=True)
        position[1] += faceDisplacement
        self.screen.blit(self.fgLeftDarkCircle, position)
        ...
        finalPupil = pygame.transform.rotate(self.fgPupil, 90)

        self.fgRightEyeImage.fill((255, 0, 66))
        self.fgRightEyeImage.blit(finalPupil, pupilPosition)
        self.fgRightEyeImage.blit(self.fgRightEyeBorder, (0,0))

        result = self.fgRightEyeMask.copy()
        result.blit(self.fgRightEyeImage, (0, 0), None, pygame.BLEND_RGBA_MULT)

        position = self.ur([1033,175], x=True, y=True)
        position[1] += faceDisplacement
        self.screen.blit(result, position)

        position = self.ur([1064,-45], x=True, y=True)
        position[1] += faceDisplacement
        self.screen.blit(self.fgRightEyebrow, position)

        position = self.ur([1043, 553], x=True, y=True)
        position[1] += faceDisplacement
        self.screen.blit(self.fgRightDarkCircle, position)


        # MOUTH
        mouthImage = self.fgMouthImage.copy()

        position = [ self.ur(65, x=True) ,0]
        position[1] -= self.fgTeethTop.get_height() * self.mouthShiftRatio * .3
        mouthImage.blit(self.fgTeethTop, position)

        position = [ 0, mouthImage.get_height() - self.fgTeethBottom.get_height() ]
        position[1] += self.fgTeethTop.get_height() * self.mouthShiftRatio * .6
        mouthImage.blit(self.fgTeethBottom, position)

        mouthImage.blit(self.fgMouthBorder, (0,0))

        result = self.fgMouthMask.copy()
        result.blit(mouthImage, (0, 0), None, pygame.BLEND_RGBA_MULT)

        scale = [ result.get_width(), result.get_height() ]
        scale[1] *= self.mouthShiftRatio * .3 + .7
        result = pygame.transform.scale(result, scale)

        position = self.ur([849,786], x=True, y=True)
        position[1] += faceDisplacement
        self.screen.blit(result, position)
